...
const { isValid, isValidPhone, isValidEmail, isValidPwd, isValidPincode } = require('../validators/validation');
...
const createUser = async (req, res) => {
    try {
        ...
        if (isValid(fname)) return res.status(400).send({ status: false, message: "First name is required and should not be an empty string" });


        if (isValid(lname)) return res.status(400).send({ status: false, message: "Last name is required and should not be an empty string" });
       ...
       if (!address.billing) return res.status(400).send({ status: false, message: "billing is required" });
        ...
        if (!isValidPhone(phone)) return res.status(400).send({ status: false, message: "Enter a valid phone number" });


        if (!isValidPwd(password)) return res.status(400).send({ status: false, message: "Password should be 8-15 characters long and must contain one of 0-9,A-Z,a-z and special characters" });


        let checkEmail = await User.findOne({ email: email });
        if (checkEmail) return res.status(409).send({ status: false, message: "Email already exist" });
